  ...
  const searchResults = useMemo(() => {
    ...
    tasks.forEach((t) => {
      if (
        ...
        results.push({
          id: `t-${t.id}`,
          title: t.title,
          subtitle: `${t.customerName ? t.customerName + ' • ' : ''}${isRtl ? 'اولویت:' : 'Priority:'} ${t.priority}`,
          module: 'TASKS',
          itemId: t.id,
    ...
    sims.forEach((s) => {
      if (
        ...
        results.push({
          id: `s-${s.id}`,
          title: s.phoneNumber,
          subtitle: `${isRtl ? 'اپراتور:' : 'Operator:'} ${s.operator} • ICCID: ${s.iccid}`,
    ...
    payments.forEach((p) => {
      if (
        ...
        results.push({
          id: `p-${p.id}`,
          title: `${isRtl ? 'رسید' : 'Receipt'} ${p.receiptNumber} (${p.amount.toLocaleString()} ${isRtl ? 'تومان' : 'Toman'})`,
          subtitle: `${p.customerName || ''} • ${isRtl ? 'روش:' : 'Method:'} ${p.method}`,
          module: 'PAYMENTS',
          itemId: p.id,
    ...
    checks.forEach((chk) => {
      if (
        ...
        results.push({
          id: `chk-${chk.id}`,
          title: `${isRtl ? 'چک صیادی' : 'Sayad Check'} ${chk.checkNumber} (${chk.bankName})`,
          subtitle: `${chk.customerName || ''} • ${isRtl ? 'مبلغ:' : 'Amount:'} ${chk.amount.toLocaleString()} ${isRtl ? 'تومان' : 'Toman'}`,
          module: 'CHECKS',
          itemId: chk.id,
    ...
    contracts.forEach((cnt) => {
      if (
        ...
        results.push({
          ...
          subtitle: `${cnt.customerName || ''} • ${isRtl ? 'نوع:' : 'Type:'} ${cnt.type}`,
          module: 'CONTRACTS',
          itemId: cnt.id,
